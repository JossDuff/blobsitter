...
/// Shared fixture for declaration-path tests: an instance with the spec's reference
...
///
/// KZG STRATEGY: forge's EVM implements the real point-evaluation precompile (0x0A),
/// and these helpers feed it the *zero-blob opening* — commitment and proof both the
/// compressed G1 point at infinity, y = 0 — which is a genuinely valid KZG opening of
/// the all-zero blob at ANY evaluation point z. Nothing on-chain checks blob *content*
/// (that is exactly the equivalence proof's job, and the proof is mocked until the
/// circuits exist), so
/// declaration-heavy tests run the real precompile with zero c-kzg tooling. Real
/// pattern-blob openings are covered by FixtureKzg.t.sol (generated by
/// tools/kzg-fixture) and re-run on a mainnet fork in test/fork/.
abstract contract InstanceTestBase is Test {
    ...
    /// The reference parameter sizing the normative spec's constants table gives.
