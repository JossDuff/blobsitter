...
# daemon itself, the protocol reference implementation, and the shared ABI.
set -euo pipefail
cd "$(dirname "$0")/.."
...
violations=$(comm -12 <(echo "$members") <(echo "$tree") \
    | grep -vx -e blobsitter-abi -e blobsitter-daemon -e blobsitter-reference || true)

if [ -n "$violations" ]; then
    echo "D6 VIOLATION: blobsitter-daemon depends on other workspace crates:" >&2
    echo "$violations" >&2
    echo "The daemon may depend only on blobsitter-reference and blobsitter-abi (never on app-layer crates)." >&2
    exit 1
fi
echo "D6 opacity check passed: daemon depends only on the protocol reference and the shared ABI."
