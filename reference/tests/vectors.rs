...
/// Blob packing must be the exact inverse relationship with `elements_from_chunks`,
/// and the versioned-hash rule must reproduce the real-KZG fixture the forge suite
/// uses (generated by tools/kzg-fixture from c-kzg's embedded Ethereum setup).
#[test]
fn blob_pack_and_versioned_hash() {
    use blobsitter_reference::blob;

    // Packing: 4097 chunks → two blobs; element u of the right blob carries chunk u
    // behind a zero high byte; the final blob's tail is all zeros.
    let chunks: Vec<Chunk> = (0..4097).map(testvec::chunk).collect();
    let blobs = blob::pack(&chunks);
    assert_eq!(blobs.len(), 2);
    for (b, raw) in blobs.iter().enumerate() {
        assert_eq!(raw.len(), blob::BLOB_BYTES);
        for e in 0..blob::FIELD_ELEMENTS_PER_BLOB {
            let u = b * blob::FIELD_ELEMENTS_PER_BLOB + e;
            let element = &raw[e * 32..(e + 1) * 32];
            if u < 4097 {
                assert_eq!(element[0], 0);
                assert_eq!(&element[1..], testvec::chunk(u as u64).as_slice(), "chunk {u}");
            } else {
                assert!(element.iter().all(|&x| x == 0), "padding element {u}");
            }
        }
    }

    // Versioned hash: anchor against the committed real-KZG fixture.
    let fixture_path =
        format!("{}/../contracts/test/fixtures/kzg_opening.json", env!("CARGO_MANIFEST_DIR"));
    let fixture: Value =
        serde_json::from_str(&std::fs::read_to_string(fixture_path).unwrap()).unwrap();
    let commitment: [u8; 48] =
        from_hex(fixture["commitment"].as_str().unwrap()).try_into().unwrap();
